...
# by np.mean(), np.std().
#========================================#
means = [np.mean(item) for item in [prec_BJ, prec_WH, prec_HK, temp_BJ, temp_WH, temp_HK]]
stds = [np.std(item) for item in [prec_BJ, prec_WH, prec_HK, temp_BJ, temp_WH, temp_HK]]
...
def hist_and_fit(time_series, var_name, mean, std):
    ...
    #========================================#
    plt.hist(data_4[var_name].values, density=True)
    ...
    y_gamma = scipy.stats.norm.pdf(x, alpha, beta)
    ax.plot(x, y_gamma)
